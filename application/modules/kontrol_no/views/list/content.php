<div class="row">
    <div class="col-md-12">
        <h4 class="m-b-lg">
            Proses Kontrol Takip Listesi
            <?php   if(isAllowedWriteModule()){ ?>
                <a href="<?php echo base_url("kontrol_no/new_form"); ?>" class="btn btn-outline btn-primary btn-xs pull-right"> <i class="fa fa-plus"></i> Yeni Ekle</a>
            <?php } ?>
        </h4>
    </div><!-- END column -->
    <div class="col-md-12">
        <div class="widget p-lg">

            <?php if(empty($items)) { ?>

                <div class="alert alert-info text-center">
                    <p>Burada herhangi bir veri bulunmamaktadır. Eklemek için lütfen <a href="<?php echo base_url("kontrol_no/new_form"); ?>">tıklayınız</a></p>
                </div>

            <?php } else { ?>           
           
                <table class="table table-hover table-striped table-bordered content-container">
                    <thead>
                        <th class="order"><i class="fa fa-reorder"></i></th>
                        <th class="w50">#id</th>
                      
                        <th>process_isim</th>
                        <th>parti_no</th>
                        <th>lot_no</th>
                        <th>kutu_no</th>
                        <th>Tarih</th>
                        <th>İşlem</th>
                    </thead>
                    <tbody class="sortable" data-url="<?php echo base_url("kontrol_no/rankSetter"); ?>">
                

                        <?php foreach($items as $item) { ?>

                            <tr id="ord-<?php echo $item->id; ?>">
                                <td class="order"><i class="fa fa-reorder"></i></td>
                                <td class="w50 text-center">#<?php echo $item->id; ?></td>
                                <td><?php echo $item->process_isim; ?></td>
                                <td><?php echo $item->parti_no; ?></td>
                                <td><?php echo $item->lot_no; ?></td>
                                <td><?php echo $item->kutu_no; ?></td>
                                <td><?php echo $item->tarih; ?></td>
                                <td class="text-center w100">
                                    <?php   if(isAllowedDeleteModule()){ ?>
                                        <button
                                            data-url="<?php echo base_url("kontrol_no/delete/$item->id"); ?>"
                                            class="btn btn-sm btn-danger btn-outline remove-btn">
                                            <i class="fa fa-trash"></i> Sil
                                        </button>
                                    <?php } ?> 
                                </td>
                            </tr>

                        <?php } ?>

                    </tbody>

                </table>

            <?php } ?>

        </div><!-- .widget -->
    </div><!-- END column -->
</div>